...
const ViewApplications = () => {
  ...
  //Function to fetch job applicants data
  ...
  <div className='flex items-center justify-center h-[70vh]'>
    ...
    <div className='container mx-auto p-4'>
      <div>
        <table className='w-full max-w-4xl bg-white border border-gray-200 max-sm:text-sm shadow'>
          <thead>
            <tr className='border-b border-gray-400'>
              <th className='py-2 px-4 text-left'>#</th>
              <th className='py-2 px-4 text-left border-b border-gray-400'>User name</th>
          ...
          <tbody>
            {applicants.filter(item => item.jobId && item.userId).map((applicant,index)=>(
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b border-gray-400 text-center flex items-center justify-start'>
                  <img className='w-10 h-10 rounded-full mr-3 max-sm:hidden' src={applicant.userId.image} alt="" />
                  <span>{applicant.userId.name}</span>
                </td>
                <td className='py-2 px-4 border-b border-gray-400 max-sm:hidden'>{applicant.jobId.title}</td>
                <td className='py-2 px-4 border-b border-gray-400 max-sm:hidden'>{applicant.jobId.location}</td>
